...
class WorkflowExecutor:
    ...
    async def run(
        ...
        on_node_complete: optional callback(node_id, NodeResult) invoked as each
        node finishes — useful for streaming progress to a UI or log.
        """
        ...
        for batch in batches:
            ...
            for node_cfg in batch:
                ...
                tasks.append(self._run_node(node_cfg, task_prompt, upstream))
            ...
            for node_cfg, result in zip(batch, results):
                ...
                if on_node_complete:
                    on_node_complete(node_cfg.id, result)
    ...
    async def _run_node(
        self, node_cfg: AgentNodeConfig, task_prompt: str, upstream: dict[str, NodeResult]
    ) -> NodeResult:
        ...
        try:
            return await agent.run(task_prompt, upstream)
